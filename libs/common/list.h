...
/** @ingroup common
 * @def LIST_INIT
 * @brief リストを初期化する。list_init関数の代替。static変数でリストを宣言した場合に便利。
 */
...
/** @ingroup common
 * @def LIST_POP_FRONT
 * @brief リストから先頭のエントリを取り出す. containerはエントリを
 * 含む構造体、fieldはlist_elem_tのフィールド名。list_pop_front関数とは
 * 違い、list_elem_tへのポインタではなくそれを内包する。
...
/** @ingroup common
 * @def LIST_CONTAINER
 * @brief list_elem_tへのポインタ (elem) から、それを含む構造体 (container) へのポインタを取得する。
 */
...
/** @ingroup common
 * @def LIST_CONTAINER
 * @brief リストの各エントリに対する、いわゆる foreach 文を実現するマクロ.
 * elem はforeach文で使う任意の変数名、list はリスト、container は
 * エントリを含む構造体、field はlist_elem_tのフィールド名。
 *
 * 使い方:
 *
 *    struct element {
 ...
 *    }
 *
 * なお __next を取り出しているのは、foreach文中で elem が削除された
 * 場合に elem->next が無効になるため。
 */
